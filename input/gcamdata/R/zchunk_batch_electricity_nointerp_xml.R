#' module_energy_batch_electricity_nointerp_xml
#'
#' Construct XML data structure for \code{electricity_nointerp.xml}.
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{electricity_nointerp.xml}. The corresponding file in the
#' original data system was \code{batch_electricity.xml.R} (energy XML).
module_energy_batch_electricity_nointerp_xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c(XML = "electricity_nointerp.xml"))
    ...
    # Produce outputs
    create_xml("electricity_nointerp.xml") %>%
      ...
      add_precursors("L223.Supplysector_elec",
                     ...
                     "L223.StubTechProd_elec",
                     "L223.StubTechEff_elec") ->
      electricity_nointerp.xml

    return_data(electricity_nointerp.xml)
  } else {
